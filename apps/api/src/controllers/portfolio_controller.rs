...
use crate::{
    config::db::AppState,
    dto::{ api_response::ApiResponse, portfolio_dto::{ CreatePortfolioDto, UpdatePortfolioDto }, user_dto::Jwt },
...
pub async fn get_portfolios_by_user_id(
    ...
    match portfolio_service::get_portfolios_by_user_id(user_id, &state.db).await {
        Ok(portfolios) => Ok(Json(ApiResponse::success("Portfolios retrieved successfully", portfolios))),
        Err(e) => {
...
pub async fn get_portfolio_by_id(
    ...
    match portfolio_service::get_portfolio_by_id(id, &state.db).await {
        Ok(Some(portfolio)) => Ok(Json(ApiResponse::success("Portfolio retrieved successfully", portfolio))),
        Ok(None) => {
            Err((StatusCode::NOT_FOUND, Json(ApiResponse::error("Portfolio tidak ditemukan"))))
        }
        Err(e) => {
...
pub async fn create_portfolio(
    ...
    match portfolio_service::create_portfolio(request, &state.db).await {
        Ok(portfolio) => Ok(Json(ApiResponse::success("Portfolio created successfully", portfolio))),
        Err(e) => {
...
pub async fn update_portfolio(
    ...
    match portfolio_service::update_portfolio(id, request, &state.db).await {
        Ok(portfolio) => Ok(Json(ApiResponse::success("Portfolio updated successfully", portfolio))),
        Err(e) => {
...
pub async fn delete_portfolio(
    ...
    match portfolio_service::delete_portfolio(id, &state.db).await {
        Ok(()) => Ok(Json(ApiResponse::success_without_data("Portfolio deleted successfully"))),
        Err(e) => {
