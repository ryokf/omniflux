...
use sea_orm::{ EntityTrait, Set, ColumnTrait, QueryFilter, prelude::Decimal };
use std::error::Error;
use chrono::Utc;
...
pub async fn gold_seeder() -> Result<(), Box<dyn Error>> {
    ...
    let existing = asset::Entity
        ::find()
        .filter(asset::Column::TickerSymbol.eq("GC=F"))
        .one(&db_conn.db).await?;
    ...
    asset::Entity::insert(active_model).exec(&db_conn.db).await?;
